...
class NST:
    ...
    def total_cost(self, generated_image):
        ...
        cost = (self.alpha * content_cost +
                self.beta * style_cost +
                self.var * var_cost)

        return cost, content_cost, style_cost, var_cost
    ...
    def compute_grads(self, generated_image):
        ...
        with tf.GradientTape() as tape:
            J_total, J_cont, J_style, J_var = self.total_cost(generated_image)
        # Compute gradients wrt input image
        # total_loss = all_loss[0]
        grad = tape.gradient(J_total, generated_image)
        return grad, J_total, J_cont, J_style, J_var
    ...
    def variational_cost(generated_image):
        ...
        loss = tf.reduce_sum(tf.image.total_variation(generated_image))
        return loss
